/* Better-Dimmer-Switch.device.groovy
 *
...
metadata {
	...
	definition (name: "Modified Better Dimmer", author: "Michael Black", namespace: "mblack01") {
	...
	tiles {
		...
		main(["switch"])
		details(["switch", "lUp", "lDown", "levelSliderControl", "lValue" , "refresh", "indicator"])
	}
}
...
def on() {
	log.info "on"
	delayBetween([zwave.basicV1.basicSet(value: 0xFF).format(), zwave.switchMultilevelV1.switchMultilevelGet().format()], 1500)
}

def off() {
	log.info "off"
	delayBetween ([zwave.basicV1.basicSet(value: 0x00).format(), zwave.switchMultilevelV1.switchMultilevelGet().format()], 1500)
}

def setLevel(value) {
	log.debug "Setting the level to: ${value}"
	delayBetween ([zwave.basicV1.basicSet(value: value).format(), zwave.switchMultilevelV1.switchMultilevelGet().format()], 1500)
}
